...
const NoteList: React.FC<NotesProp> = ({ user }) => {
 ...
 const { data: allNotesData, fetching: allNotesFetching, error: allNotesError , refetch: allRefetch} = useNoteList(user);
 const { data: filteredNotesData, fetching: filteredNotesFetching, error: filteredNotesError , refetch: filteredRefetch} = useFilterList(userID , filter)
  ...
  return (
    <main className='flex flex-col max-h-full h-[500px] max-w-full w-full overflow-y-auto bg-gradient-to-tr from-cyan-600 to-cyan-700 gap-1 px-4'>
      ...
      <ul className='w-full h-full divide-y divide-y-1'>
        ...
        : (toggleList ? <SingleNote notes={allNotesData?.todos}/>:<FilteredNote notes={filteredNotesData?.todosOfUserByStatus}/>)}
      </ul>
    </main>
  );
};

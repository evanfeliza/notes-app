...
const Navbar: React.FC<NavProp> = ({ data }) => {
  ...
  const handleDeleteUser = async () => {
    try{
      ...
      await deleteUserMutation({ id: currentUser.uid as string });
    }catch(error){
      console.log(error)
    }
  };
        ...
        <div className="flex items-center justify-between h-16 p-2">
          <div className="flex-1 flex items-center justify-center sm:items-stretch sm:justify-start">
            <div className="flex-shrink-0">
              <h3 className="text-white text-2xl md:text-4xl font-extrabold uppercase ">Notes App</h3>
            </div>
          </div>
          <div className="md:flex items-center justify-center rounded-full">
            <h1 className="hidden md:flex text-white text-lg font-semibold">{data && data.user && data.user.name}</h1>
            <button className='relative text-black p-2' onClick={() => setIsSettings(prevIsSetting => !prevIsSetting)}>
            ...
            {isSettings &&
              <ul className='z-50 absolute top-[60px] right-5 sm:right-[50px] lg:right-[200px] bg-white rounded-lg drop-shadow-md overflow-hidden divide-y divide-y-1 divide-cyan-500'>
                <li className='cursor-pointer px-4 py-1 hover:bg-slate-300 active:bg-cyan-500 ' onClick={handleSignOut}>Sign Out</li>
                <li className='cursor-pointer px-4 py-1 hover:bg-slate-300 active:bg-cyan-500' onClick={handleDeleteUser}>Delete User</li>
              </ul>}
          </div>
        </div>
      </div>

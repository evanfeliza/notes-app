...
const SingleNote: React.FC<NotesProp> = ({ notes }) => {
  ...
  return (
    <React.Fragment>
      {!updateTodoResult.fetching &&
        notes?.map((note: Note, index) => (
          <li
            key={index}
            className={`${
              note.done && `bg-slate-700 opacity-50`
            }cursor-pointer truncate max-w-full w-full grid items-center grid-cols-4 min-h-[100px] h-auto from-cyan-500 to-cyan-600 hover:bg-cyan-300 active:bg-cyan-500 duration-500 py-2`}
          >
            <div
              onClick={() => handleOpen(index as number)}
              className={`${
                note.done && `pointer-events-none`
              } flex items-center p-2 col-span-3`}
            >
            ...
            <div className="drop-shadow-md flex flex-col gap-2  md:flex-row items-center justify-center md:bg-white md:mx-auto h-auto rounded-full px-2">
              ...
              >
                {note.done ? <ImCheckboxChecked /> : <ImCheckboxUnchecked />}
              </button>

              <button
                onClick={(e) => {
                  handleDeleteNote(index as number);
                }}
      ...
      >
        <Box
          className="focus:outline-none bg-gradient-to-br drop-shadow-lg rounded-md from-cyan-500 to-cyan-400 w-[90vw] h-[90vh] md:w-4/5 md:h-2/3"
          sx={style}
        >
          ...
          <form
            ...
            className="flex flex-col gap-4 bg-inherit w-full h-[90%]"
          >
            <textarea
              className="p-4 bg-inherit focus:outline focus:outline-1 text-white focus:outline-white resize-none w-full h-full"
              {...register("editNote")}
            />
            <button
              className="focus:outline-none outline-cyan-300 bg-white p-4 rounded-md"
              type="submit"
            >
              {updateTodoResult.fetching ? (
